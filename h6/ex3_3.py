...
def update_weights(row, weights):
...
for iteration in range(n_iterations):
    ...
    weights_updates = data_rdd.map(lambda row: update_weights(row, weights_broadcast.value)).collect()
    weights = np.mean(weights_updates, axis=0) 
